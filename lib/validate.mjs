...
function ruleComponentRefs(rule) {
  ...
  if (rule.component) {
    // required-props (v0.4 §4.1) and required-composition (v0.4 §4.3, the
    // requiredCategories amendment) both anchor on nodes AT LINT TIME, where
    // sub-components appear as ordinary nodes — their `component` accepts a
    // sub-component id. The other types keep top-level-only anchors.
    const subAnchored = rule.type === "required-props" || rule.type === "required-composition";
